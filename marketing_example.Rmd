...
conv_fit_samples %>%
  ...
  tidyr::gather(Content, Estimate, -Group_B_conv, -intercept_conv) %>%
...
```{r}
spend_fit_samples %>%
  summarise(`Group A - Group B` = scales::dollar(mean((intercept_spend) - (intercept_spend + Group_B_spend))))
```
...
overall_results %>%
  ...
  tidyr::gather(Content, Estimate, -Group_B_conv, -intercept_conv, -intercept_spend, -Group_B_spend, -Sigma) %>%
  ggplot(aes(Estimate, fill = Content)) +
  geom_density(alpha = 0.5)
```
